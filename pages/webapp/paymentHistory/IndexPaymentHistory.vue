        ...
        <div class="card-body justify-content-center align-items-center">
          ...
          <v-table class="table mb-0 table-striped table-responsive-sm table-responsive-md"
            ...
            <tr v-for="order in displayData" :key="order.id">
              ...
              <td><span @click="openInfoSubs(order)" class="text-primary">{{ order.subscription.plan.name }}</span></td>
              <td><span :class="`badge bg-${ stateColor(order.state)} ms-2 text-white`">{{ stateTitle(order.state) }}</span></td>
              <td class="text-center font-weight-bold">{{ order.price_total | currency }}€</td>
              <td>{{ $dateFns.format(order.created_at, 'MMM dd yyyy')  }}</td>
